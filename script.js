...
<button onclick="myFunction()" type="submit" class="btn btn-primary button">Submit</button>
</div>`;

//creating table elements

let container = create("div", "container-lg px-5");
let row = create("div", "row mt-5");
let form = create("div", "col-lg-12 mt-2");
form.innerHTML = Data;
let tableCol = create("div", "col-lg-12 mt-2");
let table = create("table", "table");
...
var count = 0;
//function to add rows with form data in the table
function myFunction() {
  let trBody = create("tr");
  let tbId = document.createElement("td");
  tbId.setAttribute("scope", "col");
  tbId.setAttribute("class", "p-2");
  count+=1
  tbId.innerHTML = count;
  
  let tbFirstName = document.createElement("td");
  tbFirstName.setAttribute("scope", "col");
  tbFirstName.setAttribute("class", "p-2");
  var value1 = document.getElementById("inputFirstName").value
  tbFirstName.innerHTML = value1;
  
  let tbLastName = document.createElement("td");
  tbLastName.setAttribute("scope", "col");
  tbLastName.setAttribute("class", "p-2");
  var value2 = document.getElementById("inputLastName").value
  tbLastName.innerHTML = value2;
  
  let tbGender = document.createElement("td");
  tbGender.setAttribute("scope", "col");
  tbGender.setAttribute("class", "p-2");
...
for(var i = 0; i < gen.length; i++){
  ...
  tbGender.innerHTML= gender;
        
   
  let tbFood = document.createElement("td");
  tbFood.setAttribute("scope", "col");
  tbFood.setAttribute("class", "p-2");
  var food = document.getElementsByName('food');
  var dish = [];
  for(var j=0; j<food.length; j++){
    if (food[j].checked){
      var item = food[j].value;
      dish.push(item);
    }
  }
  tbFood.innerHTML = dish;
  
  let tbAddress = document.createElement("td");
  tbAddress.setAttribute("scope", "col");
  tbAddress.setAttribute("class", "p-2");
  var value3 = document.getElementById("inputAddress").value
  var value4 = document.getElementById("inputAddress2").value
  var Address = value3 + value4
  tbAddress.innerHTML = Address;
  
  let tbCity = document.createElement("td");
  tbCity.setAttribute("scope", "col");
  tbCity.setAttribute("class", "p-2");
  var City = document.getElementById("inputCity").value
  tbCity.innerHTML = City;
  
  let tbPin = document.createElement("td");
  tbPin.setAttribute("scope", "col");
  tbPin.setAttribute("class", "p-2");
  var Pin = document.getElementById("inputPin").value
  tbPin.innerHTML = Pin;
  
  let tbState = document.createElement("td");
  tbState.setAttribute("scope", "col");
  tbState.setAttribute("class", "p-2");
  var State = document.getElementById("inputState").value
  tbState.innerHTML = State;
...
function create(ele, className = "") {
  let element = document.createElement(ele);
  element.setAttribute("class", className);
  return element;
}

//appending DOM elements to table
tHead.append(trHead);
table.append(tHead, tBody);
tableCol.append(table);
row.append(form, tableCol);
